...
export default function ProfileAvatarUploader({userId,avatarUrl,name,size='lg'}:Props){
 ...
 async function upload(){
  ...
  const path=`${userId}/${crypto.randomUUID()}-${file.name.toLowerCase().replace(/[^a-z0-9._-]+/g,'-')}`;
 ...
 return <div className={`avatar-uploader avatar-${size}`}><div className="avatar-preview">{url?<img src={`${url}${url.includes('?')?'&':'?'}v=avatar`} alt="Profile picture"/>:<span>{initials}</span>}</div><div className="avatar-copy"><strong>Profile picture</strong><small>JPG, PNG or WEBP · max 5 MB</small><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>{setFile(e.target.files?.[0]??null);setMessage('')}}/><button type="button" onClick={upload} disabled={!file||busy}>{busy?'Saving…':'Save profile picture'}</button>{message&&<small role="status">{message}</small>}</div></div>
}
