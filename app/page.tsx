...
export default function Home() {
  return (
    <main>
      ...
      <section className="section" id="services"><div className="container"><p className="eyebrow">WHAT WE DO</p><h2>Build a career with clarity, confidence and global opportunity.</h2><div className="grid">{services.map(([title, text], index) => <article className="card" key={title}><span className="card-number">0{index + 1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
      ...
      <footer className="footer"><div className="container footer-inner"><img src="/careerdev-global-logo.jpg" alt="CareerDev Global" /><p>© {new Date().getFullYear()} CareerDev Global. Unleashing Your Potential.</p></div></footer>
    </main>
  );
}
