...
export default function AdminPage(){
 ...
 const [aiResult,setAiResult]=useState<any|null>(null),[adminAgents,setAdminAgents]=useState<any[]>([]),[approvalQueue,setApprovalQueue]=useState<any[]>([]),[releaseCandidates,setReleaseCandidates]=useState<any[]>([]);
 async function load(){
  ...
  if(de||oe||releaseResult.error){setMessage((de||oe||releaseResult.error)?.message||'Administration data is unavailable.');setLoading(false);return;}
  ...
  setDashboard(d as Dashboard);setOps((o||{}) as Ops);setAdminAgents((agentResult.data||[]) as any[]);setApprovalQueue(visibleApprovals as any[]);setReleaseCandidates((releaseResult.data||[]) as any[]);setLoading(false);
 }
