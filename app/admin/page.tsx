...
export default function AdminPage(){
 ...
 async function approveIndependentApproval(approvalId:string){setBusy('approve-approval-'+approvalId);setMessage('Recording independent approval…');const {error}=await supabase.rpc('approve_ai_human_approval',{p_approval_id:approvalId,p_decision_notes:'Reviewed through CareerDev Global AI Governance.'});setMessage(error?error.message:'Independent human approval recorded. The execution can now proceed through the governed completion transition.');setBusy(null);await load();} async function completeApprovedExecution(requestId:string){setBusy('complete-'+requestId);setMessage('Completing approved governed execution…');const {data,error}=await supabase.rpc('transition_ai_agent_execution',{p_request_id:requestId,p_status:'completed',p_event_data:{reason:'Independent human approval confirmed; governed test completion.'}});setMessage(error?error.message:'Governed execution completed after independent human approval.');setBusy(null);await load();}
