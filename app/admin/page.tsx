...
export default function AdminPage(){
 ...
 const [dashboard,setDashboard]=useState<Dashboard|null>(null),[role,setRole]=useState(''),[currentUserId,setCurrentUserId]=useState('');
 const [loading,setLoading]=useState(true),[message,setMessage]=useState(''),[busyUser,setBusyUser]=useState<string|null>(null);
 async function load(){
  ...
  const d=data as Dashboard;setDashboard(d);setRole(d.profiles.find(p=>p.id===userData.user.id)?.role||'');setLoading(false);
 }
 useEffect(()=>{load()},[]);
 ...
 return <main style={{minHeight:'100vh',background:'#eef7ff',color:'#09233f',padding:'36px 20px'}}><div style={{maxWidth:1240,margin:'0 auto'}}>
  <div style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'flex-start',flexWrap:'wrap'}}><div><div style={{fontWeight:800,letterSpacing:'.08em',textTransform:'uppercase',color:'#1671b9'}}>CareerDev Global</div><h1 style={{fontSize:'clamp(2rem,5vw,3.4rem)',margin:'8px 0'}}>Platform Administration</h1><p style={{maxWidth:760,lineHeight:1.7,margin:0}}>Central administration for clients, coaches, staff, administrators, marketplace operations and platform permissions.</p></div><a href="/coach-admin" style={{display:'inline-block',padding:'11px 16px',borderRadius:10,background:'#0b5d9b',color:'#fff',textDecoration:'none',fontWeight:800}}>Coach Marketplace Review →</a></div>
