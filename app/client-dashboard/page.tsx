...
export default function ClientDashboard(){
 ...
 async function load(){
  ...
  const rows=data??[];
  setBookings(rows);
  const paymentEntries=await Promise.all(rows.map(async b=>{
   const {data:payment}=await supabase.rpc('get_my_marketplace_payment_status',{p_booking_id:b.id});
   return [b.id,Array.isArray(payment)?(payment[0]??{}):{}] as const;
  }));
 ...
 async function cancel(id:string){
  ...
  setMessage('');
  const {error}=await supabase.rpc('cancel_marketplace_booking',{p_booking_id:id,p_reason:reason});
 ...
 async function preparePayment(bookingId:string){
  ...
  const {data,error}=await supabase.functions.invoke('careerdev-marketplace-payments',{body:{booking_id:bookingId,idempotency_key:`cdg-${bookingId}-${crypto.randomUUID()}`}});
  setPaymentBusy(null);
