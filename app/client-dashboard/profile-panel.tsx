...
export default function ClientProfilePanel({userId}:Props){
 ...
 return <section className="dash-card profile-account-card"><div className="account-heading"><div><p className="section-kicker">YOUR ACCOUNT</p><h2>Profile & profile picture</h2><p className="note">Keep your profile current so coaches have useful context before a session.</p></div><ProfileAvatarUploader userId={userId} avatarUrl={profile?.avatar_url} name={profile?.full_name} size="lg"/></div><div className="form-grid profile-form"><label>Full name<input value={profile?.full_name??''} onChange={e=>field('full_name',e.target.value)}/></label><label>Email<input value={registeredEmail||profile?.email||''} disabled readOnly/></label><label>Phone<input value={profile?.phone??''} onChange={e=>field('phone',e.target.value)}/></label><label>Country<input value={profile?.country??''} onChange={e=>field('country',e.target.value)}/></label><TimezoneSelect value={profile?.timezone??''} onChange={v=>field('timezone',v)} /><label>LinkedIn<input value={profile?.linkedin_url??''} onChange={e=>field('linkedin_url',e.target.value)}/></label><label className="wide">About you<textarea value={profile?.bio??''} onChange={e=>field('bio',e.target.value)} placeholder="Tell your coach a little about your goals or background."/></label></div>{(message||error)&&<div className={error?'dash-message profile-error':'dash-message'}>{error||message}</div>}<button onClick={save} disabled={busy||!profile}>{busy?'Saving…':'Save profile changes'}</button></section>
}
