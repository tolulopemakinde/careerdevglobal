...
const services = [
  "Professional CV Revamp", "LinkedIn Optimization", "Cover Letter", "Career Coaching",
  "Career Transition", "Executive Coaching", "Job Search Strategy", "Scholarship Application",
  "Fellowship Application", "Professional Essay / Personal Statement", "Interview Preparation", "Leadership Development",
];
    ...
    <main className="ci-page">
      ...
      <section id="intake" className="ci-section"><div className="ci-container">
        ...
        <div className="ci-grid">
          <div className="ci-card intake-card"><label>What service do you need?</label><select value={service} onChange={(e) => setService(e.target.value)}>{services.map((item) => <option key={item}>{item}</option>)}</select><label>Tell us what you want to achieve</label><textarea value={request} onChange={(e) => setRequest(e.target.value)} placeholder="Describe your career need, target opportunity, deadline, or challenge..." /><div className="ci-upload">📎 <strong>Upload career documents</strong><small>CV, LinkedIn export, essays, job descriptions, certificates, portfolio evidence</small></div>{!started ? <button className="ci-primary full" onClick={() => { setStarted(true); setStep(2); }}>Analyze My Request</button> : <div className="ci-success">✓ Intake started. The next question is being determined from your service request.</div>}</div>
          <div className="ci-card"><div className="ci-mini-label">DYNAMIC QUESTION ENGINE</div><h3>{started ? question : "What happens next?"}</h3>{!started ? <ol className="ci-list"><li>Understand your request</li><li>Check information already available</li><li>Analyze uploaded evidence</li><li>Ask only necessary follow-up questions</li><li>Build your Career Intelligence Profile</li></ol> : <div className="ci-question"><p>{question}</p><input placeholder="Your answer..." /><button className="ci-secondary" onClick={() => setStep(Math.min(step + 1, 5))}>Save & Continue</button></div>}<div className="ci-progress"><span style={{ width: `${step * 20}%` }} /></div><small>Step {step} of 5 · AI analyzes first; human expertise validates consequential decisions.</small></div>
        </div>
      </div></section>
