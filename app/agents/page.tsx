...
export default function AgentsPage(){
 ...
 async function run(agent:Agent){
  setRunning(agent.agent_key);setMessage('Start from Career Intelligence to provide the context this AI Agent needs.');setResult(null);window.location.href='/career-intelligence';return;
  ...
  try{
   ...
   const input={profile_id:profile?.profile_id,career_intelligence_profile:profile?.profile_data||{},agent_key:agent.agent_key,agent_name:agent.name,agent_goal:agent.description,output_schema:schema,human_review_required:false};
   ...
   setResult(body);setMessage(agent.name+' completed successfully. Review the output before consequential use; clients are encouraged to connect with a CareerDev Global Coach for validation and professional guidance.');
 ...
 return <main style={{minHeight:'100vh',background:'linear-gradient(180deg,#f7fbff,#eef6fb)',color:'#09233f',padding:'32px 16px 70px'}}><div style={{maxWidth:1200,margin:'0 auto'}}>
  ...
  {role&&<><div><section style={{background:'#fff',padding:20,borderRadius:16,border:'1px solid #b9d7ea',marginBottom:24,boxShadow:'0 8px 25px rgba(0,60,100,.04)'}}><div style={{fontSize:12,textTransform:'uppercase',letterSpacing:'.08em',fontWeight:900,color:'#1671b9'}}>Start with Career Intelligence</div><h2 style={{margin:'6px 0',fontSize:22}}>Your AI Agents work through the Career Intelligence Platform.</h2><p style={{margin:'0 0 12px',color:'#527085',lineHeight:1.6}}>AI Agents are not separate standalone tools. If you have purchased AI Agent access, your agents enhance the capacity and functionality of your Career Intelligence Platform. Start there so your career profile, goals, evidence, services and context can guide the AI-assisted work.</p><a href="/career-intelligence" style={{display:'inline-block',padding:'11px 16px',borderRadius:9,background:'#0b5d9b',color:'#fff',textDecoration:'none',fontWeight:900}}>Open Career Intelligence →</a></section></div>{role&&<><section style={{background:'#fff',padding:18,borderRadius:16,border:'1px solid #d8e6f1',marginBottom:24}}><div style={{fontSize:12,textTransform:'uppercase',letterSpacing:'.08em',fontWeight:900,color:'#1671b9'}}>Shared intelligence layer</div><h2 style={{margin:'6px 0'}}>Career Intelligence Profile™</h2><p style={{margin:0,color:'#527085'}}>All {agents.length} agents use the same evolving profile context. Each run is governed, logged, structured, and returned as AI decision-support. Coaches can review outputs before professional use.</p>{subscription?<p style={{margin:'10px 0 0',fontWeight:800}}>Current plan: {subscription.plan_name} · {subscription.monthly_credits.toLocaleString()} AI credits/month allowance</p>:<p style={{margin:'10px 0 0',color:'#8a5b00',fontWeight:800}}>No active AI Agent subscription. <a href="/pricing" style={{color:'#0b5d9b'}}>Choose a plan →</a></p>}</section>
   {loading?<p>Loading agents…</p>:Object.entries(grouped).map(([cat,list])=><section key={cat} style={{marginBottom:28}}><h2 style={{fontSize:22}}>{groups[cat]||cat}</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:14}}>{list.map(a=>{const allowed=included.has(a.agent_key);return <article key={a.id} style={{background:'#fff',border:'1px solid '+(allowed?'#d8e6f1':'#e3e8ed'),borderRadius:16,padding:18,boxShadow:'0 8px 25px rgba(0,60,100,.05)',opacity:allowed?1:.86}}><div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'start'}}><h3 style={{margin:0}}>{a.name}</h3><span style={{fontSize:11,fontWeight:900,padding:'4px 7px',borderRadius:999,background:allowed?'#eaf6ef':'#f1f3f5',whiteSpace:'nowrap'}}>{allowed?'Included':'Locked'}</span></div><p style={{color:'#527085',lineHeight:1.55,minHeight:66}}>{a.description}</p><button onClick={()=>allowed&&run(a)} disabled={running!==null||!allowed} style={{width:'100%',padding:'11px 14px',border:0,borderRadius:9,background:allowed?'#0b5d9b':'#dfe6ec',color:allowed?'#fff':'#527085',fontWeight:900,cursor:allowed?'pointer':'default'}}>{allowed?'Start in Career Intelligence →':'Upgrade to Access'}</button>{!allowed&&<a href="/pricing" style={{display:'block',textAlign:'center',marginTop:9,color:'#0b5d9b',fontWeight:800,fontSize:12}}>View plans</a>}<small style={{display:'block',marginTop:9,color:'#718394'}}>AI output: Ready for use as decision-support · Coach validation recommended for clients</small></article>})}</div></section>)}
  ...
  <footer style={{marginTop:45,paddingTop:20,borderTop:'1px solid #d8e6f1',fontSize:12,color:'#718394'}}>CareerDev Global AI Agents provide decision support. Clients should consider connecting with a CareerDev Global Coach to validate results and determine appropriate next steps before consequential professional use.</footer>
 </div></main>
}
