...
export default function ProfessionalStandardsCarousel() {
  ...
  return (
    <section id="professional-standards" className="professional-standards" aria-labelledby="professional-standards-title">
        ...
        <div className="standards-viewport" aria-label="Professional bodies CareerDev Global aligns its practice with">
          <div className="standards-track">
            {items.map((standard, index) => (
              <a className="standard-logo" href={standard.href} target="_blank" rel="noreferrer" aria-label={`${standard.name} — official website`} key={`${standard.short}-${index}`}>
